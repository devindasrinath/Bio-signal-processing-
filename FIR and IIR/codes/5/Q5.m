...
fc_comb = 50; %50Hz
q_comb = 35; %Q factor
bw_comb = (fc_comb/(fs/2))/q_comb;
% %================Realising IIR LP filters==================================
%the higher order cases to unsatbailty IIR filteres .Therefore we chose M
%as 10.
...
%LPF M = 10
[coff_n_low,coff_d_low] = butter(M , (2* fc_low) /fs , 'low');
fvtool(coff_n_low,coff_d_low);

%HPF M = 10
[coff_n_high,coff_d_high] = butter(M , (2* fc_high) /fs , 'high');
fvtool(coff_n_high,coff_d_high);

%comb M = 10
[coff_n_comb,coff_d_comb]=iircomb(fs/fc_comb,bw_comb ,'notch');
